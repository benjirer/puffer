...
function AVSource(ws_client, server_init) {
  ...
  worker.onmessage = function (e) {
    ...
    switch (type) {
      case 'fatal_error':
        set_fatal_error(data.error_message);
        report_error(0, data.error_message);
        break;
      case 'video_ack':
        ws_client.send_client_ack('client-vidack', data);
        break;
      case 'audio_ack':
  ...
  this.isOpen = function () {
    return true; // the worker handles open state
  };
