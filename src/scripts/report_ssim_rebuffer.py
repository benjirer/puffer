...
def report_ssim_rebuffer(curr_ts, days):
    ...
    time_range = "{}_{}".format(
        start_ts.strftime(short_time_str), curr_ts.strftime(short_time_str)
    )
    ...
    cmd = [
        ...
        output_fig,
        "--from",
        start_ts.strftime(time_str),
        "--to",
        curr_ts.strftime(time_str),
    ]
    ...
    gs_url = "https://storage.googleapis.com/{}/{}".format(
        bucket_folder, output_fig_name
    )
