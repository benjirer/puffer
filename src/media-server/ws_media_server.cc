...
void handle_client_info(WebSocketClient &client, const ClientInfoMsg &msg)
{
  ...
  if (msg.screen_width and msg.screen_height)
  {
    ...
    if (enable_logging)
    {
      string log_line = to_string(timestamp_ms()) + "," + server_id + "," + expt_id + "," + client.username() + "," + to_string(client.first_init_id().value()) + "," + to_string(msg.init_id) + "," + client_address_with_port(client) + "," + client.os() + "," + client.browser() + "," + to_string(*msg.screen_width) + "," + to_string(*msg.screen_height);
      append_to_log("client_sysinfo", log_line);
    }
  }
